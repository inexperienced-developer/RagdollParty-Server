using InexperiencedDeveloper.Multiplayer.Riptide.ServerDev;
...
namespace InexperiencedDeveloper.Core.Controls
{
    public class PlayerControls : MonoBehaviour
    {
        ...
        {
            print($"My lobby: {Player.NetPlayer}");
            print($"Players in game: {Player.NetPlayer.MyLobby.PlayersInGame}");
            foreach (var player in Player.NetPlayer.MyLobby.PlayersInGame)
            {
                SendServerPos(Player.NetPlayer.Id, player.Id);
            }
        }
        ...
        private void SendServerPos(ushort playerId, ushort toPlayerId)
        {
            if (NetworkManager.Instance.CurrentTick % 3 != 0) return;
            Message msg = Message.Create(MessageSendMode.unreliable, (ushort)ServerToClientCommand.SyncPosition);
            msg.AddUShort(playerId);
            //msg.AddUShort(NetworkManager.Instance.CurrentTick);
            //msg.AddBool(DidTeleport);
